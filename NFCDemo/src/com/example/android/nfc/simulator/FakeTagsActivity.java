...
public class FakeTagsActivity extends ListActivity {
	...
	public static NdefRecord newMimeRecord(String type, byte[] data) {
		...
		return new NdefRecord(NdefRecord.TNF_MIME_MEDIA, typeBytes,
				new byte[0], data);
	}
	...
	static final class TagDescription {
		...
		public TagDescription(String title, byte[] bytes) {
			...
			try {
				...
				throw new RuntimeException("Failed to create tag description",
						e);
			}
		}
	...
	public void onCreate(Bundle savedState) {
		...
		adapter.add(new TagDescription("Broadcast NFC Text Tag",
				MockNdefMessages.ENGLISH_PLAIN_TEXT));
		adapter.add(new TagDescription("Broadcast NFC SmartPoster URL & text",
				MockNdefMessages.SMART_POSTER_URL_AND_TEXT));
		adapter.add(new TagDescription("Broadcast NFC SmartPoster URL",
